function [bootLevelVals] = dispBootstrap(VAL_OBS, VAL_BOOT, DISP_SIG_LEVELS)
%function [bootLevelVals] = dispBootstrap(VAL_OBS, VAL_BOOT, DISP_SIG_LEVELS)
...
%
% Output:
% bootLevelVals --> thresholds for p values [.05, .01, .001]
%
% History:
% --------
...
xMin    = min([VAL_OBS VAL_BOOT]) - .2*abs(min([VAL_OBS VAL_BOOT]));
xMax    = max([VAL_OBS VAL_BOOT]) + .2*abs(max([VAL_OBS VAL_BOOT]));
binStep = (xMax-xMin)/100;
bins    = [xMin:binStep:xMax];
...
if (DISP_SIG_LEVELS ==1)
    ...
    fill([xi(xi>bootLevelVals(1)) xi(end) xi(find(xi>bootLevelVals(1),1))]+.5*binStep, ...
            [area*fi(xi>bootLevelVals(1)) 0 0] , [255 255 100]./255, 'FaceAlpha', 0.7); 
    fill([xi(xi>bootLevelVals(2)) xi(end) xi(find(xi>bootLevelVals(2),1))]+.5*binStep, ...
            [area*fi(xi>bootLevelVals(2)) 0 0] , [255 100 50]./255, 'FaceAlpha', 0.8); 
    fill([xi(xi>bootLevelVals(3)) xi(end) xi(find(xi>bootLevelVals(3),1))]+.5*binStep, ...
...
plot(xi+.5*binStep,area*fi,'color','k','linewidth',1);
hold off
