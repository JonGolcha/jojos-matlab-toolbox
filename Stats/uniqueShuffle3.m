function [S] = uniqueShuffle3(N_SHUFFLE, NB_WIN, NB_SIG, BINARY_OUTPUT)
% function  [S] = uniqueShuffle3(N_SHUFFLE, NB_WIN, NB_SIG, BINARY_OUTPUT)
%
% ************************************************************************
% This function return a random permutation matrices of size 
% (N_SHUFFLE,NB_WIN,NB_SIG) with only unique rows filled with random indexes 
% in range [1->NB_WIN] and no identical indexes for each corresponding row
% in the third dimension.
% e.g., having S(1,:,1) = [ 1 2 3 ; 1 3 2] and S(1,:,2) = [ 3 2 1 ; 2 1 3] is forbidden
% due to value 2 in first row.
%
% Main purpose is to create surrogate data from multiple vectors of identical 
% length NB_WIN. These vectors can be for example trial indexes for each channel 
% in some experiment.
%
% 
%
...
% NB_SIG        --> scalar (must be less than NB_WIN to ensure unique permutations)
...
% History:
% --------
% *** 2012-01-14, update on 2016-06-21 to add multiple output indexes
% Created by J.Chatel-Goldman @GIPSA Lab, jonas.chatel.goldman(at)gmail.com


if (nargin<3)||isempty(NB_SIG)
   NB_SIG = 1; 
end
if (nargin<4)||isempty(BINARY_OUTPUT)
   BINARY_OUTPUT = 0; 
end
if NB_SIG > NB_WIN
    error('impossible to make unique permutations when NB_SIG > NB_WIN'),
end
...
S = zeros(N_SHUFFLE, NB_WIN, NB_SIG);
S(:,:,1) = S1;
for sig_ix = 2:NB_SIG
    allUnique = false;
    while(~allUnique)
...
if BINARY_OUTPUT
    S = S > NB_WIN/2; % set half of random shuffling values to zero, and half to one.
end
