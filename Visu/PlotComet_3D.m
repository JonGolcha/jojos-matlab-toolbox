...
% tailFormat = struct('LineWidth',1,'LineStyle','-');
% headFormat = struct('LineStyle','none','Marker','o','MarkerSize',6);
...
if ~exist('cFigure')
    ...
    view(3);
    axis('equal'),
%     axis([min(min(x_data)), max(max(x_data)),...
%         min(min(y_data)), max(max(y_data)),...
%         min(min(z_data)), max(max(z_data))]);
end
...
for n = 1:1:length(x_data)
    ...
    plot3(x_data(n_start:n_stop,:),...
        ...
        z_data(n_start:n_stop,:),tailFormat);
    plot3(x_data(n_stop,:), y_data(n_stop,:), z_data(n_stop,:),headFormat);
    grid on, xlabel(labels(1)), ylabel(labels(2)), zlabel(labels(3)),
    title(title_str),
    drawnow;
